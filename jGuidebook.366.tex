...
%The game will be restored automatically the next time you play
ゲームをセーブする(プレイを中断してプログラムを終了する)。
ゲームは次回プレイ時に自動的に復旧される。\\
%%.lp ""
%%.lp ""
\item[\tb{t}]
...
%By default a response of `{\tt y}' acknowledges that intent.
%You can set the
%{\it paranoid\verb+_+confirmation\/}
...
%It usually feeds itself on fresh carrion and other meats.
...
%Some wands are directional---you must give a direction in which to zap them.
%You can also
...
杖は通常何回も魔力を発揮する．杖の中には方向性のあるものがあり、
...
例えば {\it autoquiver} をオン、{\it autopickup\/}をオフ、
{\it name\/}が``Blue Meanie'' に、{\it fruit\/}が`` papaya''になるように
...
このオプションは矢筒(または矢筒袋または準備している物)が空の時に`f'
...
%It is implemented only by the tty port (default off), when the game has been compiled to support tty graphics.
...
%NetHack should display this number of messages at a time
...
%At this time the tty, win32tty and
...
%Note that the platform used may interpret the
...
%You can also override one or more symbols using the {\it SYMBOLS\/}
...
%play in wizard mode (the debugging mode, not the magic-useing role). 
%A value of a single
...
%announcement was posted on the {\it NetHack Development Team}'s official
%nethack.org website to that effect,
%stating that there would never be a 3.4.4, 3.5, or 3.5.0
...
{\it NetHack Development Team} の公式 nethack.org web サイトにアナウンスが投稿され、
...
%%.pg
\medskip
%The 3.6 version merges work done by NetHack development
...
3.6 バージョンでは、3.4.3 リリースから開発チームによってなされた作業と、
...
%Gods of the Dungeon sometimes makes note of the names of the worst
