...
きる。また、環境変数``NETHACKOPTIONS''や設定ファイルで自動的に設定する
...
{\it NetHack\/}3.1 を作成した。
{\it Warwick Allison} は Atari 用に NetHack のタイル版を作成した。
...
{\it Warwick Allison} は NetHack を Qt インターフェースを使うように
変更した。
...
Kevin は後に開発チームに加わり、これらのアイデアを NetHack 3.3 に
統合した。
...
NetHack 3.4.0 リリース直前の 2002 年 3 月に
...
\nd {\it Michael Allison} は MS-DOS 版の NetHack 3.4 を管理している。
...
%all these years.
\nd {\it Ron Van Iwaarden} は過去いくつかのリリースの OS/2 版 NetHack の
唯一の管理者だった。
...
{\it Kevin Smolkowski} は NetHack 3.6.0 が様々な Unix の亜種で
